...
* **Float number recognition**: Distinguishing between the decimal point in a float and a dot operator (if it existed) required careful lookahead.
* **Keyword vs identifier**: Implementing the lookup table for keywords while still allowing custom identifiers.
* **Position tracking**: Maintaining accurate line and column numbers while reading characters and skipping whitespace.
